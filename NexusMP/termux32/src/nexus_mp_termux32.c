...
int nexus_mp_start_server(nexus_mp_server_t *server) {
    ...
    /* ARM32: bind() needs privileged port — root is required */
    ...
    /* Backlog reduced for 32-bit mobile constraints */
    if (listen(server->listen_socket, 32) < 0) {
        perror("listen");
        close(server->listen_socket);
        return -1;
    }
    ...
    server->running = 1;
    printf("[%s] NexusMP Termux32 server listening on port %u\n",
...
int nexus_mp_handle_client(client_connection_t *client) {
    ...
    nexus_mp_send_response(client->socket_fd,
        "* OK [CAPABILITY IMAP4rev1 IDLE STARTTLS COMPRESS=DEFLATE] NexusMP Termux32 ready\r\n");
    ...
    while (1) {
        ...
        if (strstr(cmd, "CAPABILITY")) {
            ...
            nexus_mp_send_response(client->socket_fd,
                "* BYE NexusMP Termux32 closing connection\r\n"
                "OK LOGOUT completed\r\n");
            free(cmd);
            break;
        } else {
