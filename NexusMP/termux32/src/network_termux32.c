...
network_listener_t32_t *network_create_listener_t32(const char *bind_ip, uint16_t port, int backlog) {
    ...
    network_listener_t32_t *listener = malloc(sizeof(network_listener_t32_t));
...
int network_start_listening_t32(network_listener_t32_t *listener) {
    ...
    if (bind(listener->server_socket,
             (struct sockaddr *)&listener->bind_addr,
             sizeof(listener->bind_addr)) < 0) {
        perror("bind");
        close(listener->server_socket);
        listener->server_socket = -1;
        pthread_mutex_unlock(&listener->lock);
...
network_connection_t32_t *network_accept_connection_t32(network_listener_t32_t *listener) {
    ...
    int fd = accept(listener->server_socket, (struct sockaddr *)&client_addr, &addr_len);
    if (fd < 0) {
        perror("accept");
        return NULL;
    }

    network_connection_t32_t *conn = malloc(sizeof(network_connection_t32_t));
    if (!conn) {
        close(fd);
        return NULL;
    }
...
ssize_t network_receive_t32(network_connection_t32_t *conn, void *buffer, size_t len) {
    ...
    if (n > 0)
        conn->bytes_received += (uint32_t)n;
...
int network_set_keepalive_t32(network_connection_t32_t *conn, int keepalive) {
    ...
    return setsockopt(conn->socket_fd, SOL_SOCKET, SO_KEEPALIVE, &keepalive, sizeof(keepalive));
}
